...
carousels.forEach(carousel => {
    ...
    const showImage = (index) => {
        ...
        // Atualiza somente se for um link e tiver URL
        if (visitBtn && visitBtn.tagName === 'A' && newUrl) {
    ...
    // --- NAVEGAÇÃO POR CLIQUE (SETAS - DESKTOP) ---
    if (nextBtn) {
    ...
    // --- NAVEGAÇÃO POR SWIPE (TOUCH - MOBILE) ---
    let touchStartX = 0;
    let touchEndX = 0;
    ...
    function handleSwipe() {
        // Se arrastou para a ESQUERDA (Próximo)
        if (touchEndX < touchStartX - 50) {
            currentIndex = (currentIndex + 1) % images.length;
            showImage(currentIndex);
        }
        // Se arrastou para a DIREITA (Anterior)
        if (touchEndX > touchStartX + 50) {
            currentIndex = (currentIndex - 1 + images.length) % images.length;
            showImage(currentIndex);
        }
...
function updateModal() {
    ...
    const isEnglish = document.querySelector(".language-toggle h1").textContent === "BR";
